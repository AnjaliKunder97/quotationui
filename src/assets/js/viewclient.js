$(window).ready(function(){
    ...
    $('.actionrow a').attr('href', 'AddClient.html?client=' + id + '&legal=' + legalstatus)
    if(legalstatus){
        switch(legalstatus){
            case '1':
    ...
    $.ajax({
        url:urlRoot + 'clients/'+T_legalstatus+'/'+id + '?display=true',
        type: 'GET',
        dataType: 'json',
        ...
        error:function(error){
            if(error.detail.include("No credentials provided")){
                swal({
                    title: "Sorry",
